...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	...
	p := a.Progress()
	cache := "N/A"
	if m, ok := a.store.CacheInfo(); ok {
		age := int(time.Since(m).Seconds())
		cache = fmt.Sprintf("%dh %dm", age/3600, age%3600/60)
	}
	...
	fmt.Fprintf(w, "\nCached items: %d\nCache age:    %s\nCache TTL:    %gh\n\nPort:  %d\nFeed:  %s/feed.xml\n",
		len(a.store.Releases()), cache, cfg.CacheTTLHours, a.port, cfg.PublicBaseURL)
}
...
func (s *Server) status(w http.ResponseWriter, r *http.Request) {
	...
	out := map[string]any{
		...
		"cache": map[string]any{
			"items": len(rels), "with_description": withDesc, "failed": failed, "ttl_hours": cfg.CacheTTLHours,
		},
		"now": time.Now(),
	}
	...
	if m, ok := a.store.CacheInfo(); ok {
		out["cache"].(map[string]any)["mtime"] = m
		out["cache"].(map[string]any)["fresh"] = a.store.CacheFresh(cfg.CacheTTLHours)
	}
...
func (s *Server) releases(w http.ResponseWriter, r *http.Request) {
	...
	onlyFail := r.URL.Query().Get("failed") == "1"
	rels := s.app.store.Releases()
	var out []relSummary
	for _, x := range rels {
		if q != "" && !strings.Contains(strings.ToLower(x.Title), q) {
			continue
		}
		if tag != "" {
			found := false
		...
		if onlyFail && x.EnrichError == "" {
			continue
		}
		out = append(out, relSummary{
			Title: x.Title, Link: x.Link, Pub: x.PubDateISO, Categories: x.Categories, Cover: s.coverURL(x),
			Images: len(x.ImageURLs), DescLen: len(x.ExtraDescription), Error: x.EnrichError, EnrichedAt: x.EnrichedAt,
		})
	}
	switch sortBy {
	case "title":
	...
	case "oldest":
		sort.SliceStable(out, func(i, j int) bool { return out[i].Pub < out[j].Pub })
	default:
		sort.SliceStable(out, func(i, j int) bool { return out[i].Pub > out[j].Pub })
	}
	total := len(out)
	if l, _ := strconv.Atoi(r.URL.Query().Get("limit")); l > 0 && len(out) > l {
		out = out[:l]
	}
	if out == nil {
		out = []relSummary{}
	}
	writeJSON(w, 200, map[string]any{"total": total, "items": out})
}

func (s *Server) releaseDetail(w http.ResponseWriter, r *http.Request) {
	rel, ok := s.app.store.Find(r.URL.Query().Get("link"))
	if !ok {
...
func (s *Server) tags(w http.ResponseWriter, r *http.Request) {
	cnt := map[string]int{}
	for _, x := range s.app.store.Releases() {
		for _, c := range x.Categories {
			cnt[c]++
		}
	}
...
func (s *Server) clearCache(w http.ResponseWriter, r *http.Request) {
	...
	s.app.store.ClearCache()
	s.app.log.Warn("Release cache cleared from web UI (feed.xml kept until next run)")
	writeJSON(w, 200, map[string]any{"ok": true})
}
